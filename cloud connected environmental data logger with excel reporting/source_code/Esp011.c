...
//		return;
	...
	string_lcd("AT+CWJAP");
	delay_ms(1000);
	string_uart("AT+CWJAP=\"SAI\",\"123456789\"\r\n");
	i=0;memset(buff,'\0',200);
...

{
		...
		if(field==1)
		{
			string_uart("GET /update?api_key=1XDU151LIUPCF51J&field1=");
			cmd_lcd(0x01);
			cmd_lcd(0x80);
		...
		{
	string_uart("GET /update?api_key=1XDU151LIUPCF51J&field2=");//LUCKY API KEY
		cmd_lcd(0x01);
		...
		{
	string_uart("GET /update?api_key=1XDU151LIUPCF51J&field3=");
				  	  cmd_lcd(0x01);
             cmd_lcd(0x80);
